...
public class PageStateView extends FrameLayout {
    ...
    public PageStateView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.PageStateView);
        int loadingViewResId = a.getResourceId(R.styleable.PageStateView_loading_view, -1);
        if (loadingViewResId > -1) {
            mLoadingView = mInflater.inflate(loadingViewResId, this, false);
            addView(mLoadingView, mLoadingView.getLayoutParams());
        }
        int emptyViewResId = a.getResourceId(R.styleable.PageStateView_empty_view, -1);
        if (emptyViewResId > -1) {
            mEmptyView = mInflater.inflate(emptyViewResId, this, false);
            addView(mEmptyView, mEmptyView.getLayoutParams());
        }
        int errorViewResId = a.getResourceId(R.styleable.PageStateView_error_view, -1);
        if (errorViewResId > -1) {
            mErrorView = mInflater.inflate(errorViewResId, this, false);
            addView(mErrorView, mErrorView.getLayoutParams());
        }
        int viewState = a.getInt(R.styleable.PageStateView_view_state, VIEW_STATE_CONTENT);
        switch (viewState) {
